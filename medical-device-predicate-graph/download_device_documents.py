...
def download_url(url: str) -> None:
    ...
    if resp.status_code != 200:
        print(
            "Failed downloading url because of non-200 http status code: url='{}' status_code='{}'".format(
        ...
        print(
            "Failed downloading url because of non-pdf file : url='{}' content_type='{}'".format(
...
def main(gcs_bucket: str, gcp_project: str, dst_folder: str, since: str, dry_run: str):
    ...
    downloaded_device_ids = set(
        [blob.name[4:-4] for blob in client.list_blobs(gcs_bucket, prefix="raw")]
    )
